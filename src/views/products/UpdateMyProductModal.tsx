...
export default function UpdateMyProductModal({
  ...
  const [deletedImages, setDeletedImages] = useState(false) // Nuevo estado para controlar eliminación de imágenes
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    // Manejo de imágenes
    if (deletedImages) {
      // Si el usuario eliminó las imágenes, enviamos un archivo vacío para indicar eliminación
      formData.append('Images', new File([], ''))
    } else if (data.images && data.images.length > 0) {
      // Si hay nuevas imágenes subidas, las agregamos al FormData
      for (let i = 0; i < data.images.length; i++) {
        formData.append('Images', data.images[i])
      }
    } else if (existingImages.length > 0) {
      // Si no hay imágenes nuevas pero hay existentes, convertimos las URLs a Files
      try {
        // Procesamos las primeras 5 imágenes como máximo para no sobrecargar
        const imagesToProcess = existingImages.slice(0, 5)

        for (let i = 0; i < imagesToProcess.length; i++) {
          const imageFile = await getFileFromImageUrl(imagesToProcess[i], `imagen_${i}.jpg`, 'image/jpeg')

          formData.append('Images', imageFile)
        }
      } catch (error) {
        console.error('Error al procesar las imágenes existentes:', error)
        toast.error('Error al procesar las imágenes. Por favor, suba nuevas imágenes.')
        setLoading(false)
  ...
  // Añadir función para eliminar todas las imágenes
  ...
  const handleModalClose = () => {
    ...
    setExistingImages([])
    setNameError(null)
    setDeletedImages(false) // Resetear el estado de eliminación
    handleClose()
  }
        ...
        <form id='update-product-form' onSubmit={handleSubmit(onSubmit)}>
          ...
          <Button variant='outlined' component='label' fullWidth sx={{ mb: 1 }}>
            ...
            <input
              ...
              {...register('images', {
                onChange: () => {
                  // Al subir nuevas imágenes, desactivamos el flag de eliminación
                  setDeletedImages(false)
                }
              })}
            />
          </Button>
          {imagesPreview.length > 0 && (
            <Button variant='outlined' onClick={() => resetField('images')} fullWidth sx={{ mb: 1 }}>
              Eliminar Nuevas Imágenes
            </Button>
          )}
          {!deletedImages && existingImages.length > 0 && (
            <Button variant='outlined' onClick={handleDeleteAllImages} fullWidth sx={{ mb: 2 }} color='error'>
